...
Addon.Nameplates.FrameCastBar = Frame
    :IgnoreParentAlpha(true)
{
    ...
    [Script.OnUpdate] = function(self)
        ...
        if self.Bar:IsShown() then
            ...
            if self.channeling then
                ...
                self.Bar:SetWidth(progress * self:GetWidth())
            end
        end
    ...
    Bar = Frame
        .TOPLEFT:TOPLEFT(PARENT:GetParent().Health)
        .BOTTOMLEFT:BOTTOMLEFT(PARENT:GetParent().Health)
        :Hide()
    {
        Spark = Texture
            .CENTER:RIGHT()
