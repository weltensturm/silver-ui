...
---@type table<string, Enum.PowerType>
local BuffPowerEnum = {
    Icicles = 100000 --[[@diagnostic disable-line: assign-type-mismatch]]
}
...
---@type table<table, Enum.PowerType|nil>
local BuffPowerMap = {
    [{ class='MAGE', spec=3 }] = IsRetail and BuffPowerEnum.Icicles or nil
}


---@return Enum.PowerType|nil
local function UnitSecondaryPowerType(unit)
    ...
    for _, power in pairs(Enum.PowerType) do
        if
            ...
            and power ~= Enum.PowerType.AlternateQuest
            and power ~= Enum.PowerType.AlternateEncounter
            and (power ~= Enum.PowerType.AlternateMount or IsMounted())
            and power ~= mainPower
...
local SecondaryPowerBar = Frame {

    background = Texture
        :AllPoints()
        :Texture 'Interface/AddOns/silver-ui/art/playerframe-secondarybar-bg'
    ...
    border = Texture
        :AllPoints()
        :Texture 'Interface/AddOns/silver-ui/art/playerframe-secondarybar'
        :VertexColor(0.1, 0.1, 0.1, 0.7)
        
}


Addon.Units.PlayerSecondaryPower = Frame { LQT.UnitEventBase }
    ...
    :IsFrameBuffer(true)
{
    ...
    bar = SecondaryPowerBar:AllPoints(),
    ...
    Update = function(self)
        if self.power then
            ...
            if self.discrete then
                ...
                self.bar.bar:SetWidth(current/mod / max * 210)
            end
        end
    end,
